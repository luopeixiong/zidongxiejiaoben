def html(self, response):
    if response.status == 200 and len(response.text) > 10:
        ...
        items['title'] = rs['']
        items['content'] = rs['']
        time_text = re.findall(r"\d{4}-\d{1,2}-\d{1,2}|\d{4}/\d{1,2}/\d{1,2}|\d{4}.\d{1,2}.\d{1,2}|\d{4}年\d{1,2}月\d{1,2}", str(rs['']))
        if time_text:
            items['publishtime'] = time_text[0].replace('.', '-').replace(' ', '').replace('/', '-').replace('年', '-').replace('月', '-')
