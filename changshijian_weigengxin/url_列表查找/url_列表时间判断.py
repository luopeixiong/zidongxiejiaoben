...
for txt_path in natsort.natsorted(liebiao_txt_path_lst):
    ...
    for url_siweitime_erweitime_str in url_siweitime_erweitime_str_lst:
        if url_siweitime_erweitime_str.count(',') != 2:
            duoyu = url_siweitime_erweitime_str.count(',') - 2
            url_siweitime_erweitime_str = url_siweitime_erweitime_str.replace(',', '', duoyu)
        if url_siweitime_erweitime_str != '':
            ...
            if url[-1] == '/':
                ...
                if siweitime_erweitime_str not in url_qita.keys():
                    url_qita[siweitime_erweitime_str] = []
                    url_qita[siweitime_erweitime_str].append(url)
                else:
                    url_qita[siweitime_erweitime_str].append(url)
    # qita_url排序
    for k, v in url_qita.items():
        url_qita[k] = sorted(v, key=lambda i: len(i), reverse=False)
