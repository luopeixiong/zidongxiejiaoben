...
class MyWindow(QWidget):
    ...
    def def0(self):
        ...
        yuming_LineEdit = QLineEdit()
        yuming_LineEdit.setGeometry(55, 20, 200, 20)
        yuming_LineEdit.setText(self.kongjian_text_data['脚本域名'])
        shurukuang_lst.append(yuming_LineEdit)
        self.kongjian_dict['域名_lst'] = yuming_LineEdit
    ...
    def def2(self):
        label_lst = ['列表title-xpath：', '列表url-xpath：', '列表时间-xpath：', '列表时间-re：', 'url拼接：【不用写，挂着】']
        shurukuang_lst = []
        ...
        liebiao_publishtime_re_LineEdit = QLineEdit()
        liebiao_publishtime_re_LineEdit.setText("-2")
        liebiao_publishtime_re_LineEdit.setGeometry(55, 20, 200, 20)
        shurukuang_lst.append(liebiao_publishtime_re_LineEdit)
        self.kongjian_dict['url拼接'] = liebiao_publishtime_re_LineEdit
    ...
    def def4(self):
        ...
        self.fanye_fangan_wenjianjia_name_ComboBox.addItems(['fanye1'])
        # 信号
        # self.cb.currentIndexChanged[str].connect(self.click_my_btn)  # 条目发生改变，发射信号，传递条目内容
    ...
    def click_my_btn(self, i):
        ...
        if i == 1:
            self.def3_zhaobiao_urlpanduan_biaotipanduan_LineEdit.setEnabled(True)
        elif i == 2:
            self.def3_zhaobiao_urlpanduan_biaotipanduan_LineEdit.setEnabled(False)
        print('---------------')
    ...
    def baocun_Data(self):
        with open("gui_data.json", "r", encoding="utf-8") as f:
            ...
            old_data["脚本域名"] = self.kongjian_dict['域名_lst'].text()
