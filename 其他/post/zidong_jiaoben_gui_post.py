...
class MyWindow(QWidget):
    ...
    def init_ui(self):
        with open('post_data.json', 'r', encoding='utf8') as f:
            self.kongjian_text_data = json.load(f)
        self.setWindowTitle('post一般模板')
        self.resize(500, 500)
        self.kongjian_dict = {}
    ...
    def def1(self):
        qishi_url_LineEdit = QLineEdit()
        qishi_url_LineEdit.setGeometry(55, 20, 200, 20)
        qishi_url_LineEdit.setText(self.kongjian_text_data['起始url'])
        self.kongjian_dict['起始url'] = qishi_url_LineEdit
        label = QLabel('脚本起始url')
        self.layout1.addWidget(label)
        self.layout1.addWidget(qishi_url_LineEdit)

        qishi_data_TextEdit = QTextEdit()
        qishi_data_TextEdit.setGeometry(55, 20, 200, 20)
        qishi_data_TextEdit.setAcceptRichText(False)
        qishi_data_TextEdit.setFontFamily("黑体")  # 字体设置为黑体
        qishi_data_TextEdit.setText(self.kongjian_text_data['起始data_lst'])
        self.kongjian_dict['起始data_lst'] = qishi_data_TextEdit
        label = QLabel('脚本起始data')
        self.layout1.addWidget(label)
        self.layout1.addWidget(qishi_data_TextEdit)

    def def2(self):
        label_lst = ['列表定位—dict', '列表title-dict：', '列表url-dict：', '列表url-拼接前段【可为空】', '列表时间-dict：', '列表时间-re：']
        shurukuang_lst = []

        liebiao_biaoti_xpath_LineEdit = QLineEdit()
        liebiao_biaoti_xpath_LineEdit.setText(self.kongjian_text_data['列表定位—dict'])
        liebiao_biaoti_xpath_LineEdit.setGeometry(55, 20, 200, 20)
        shurukuang_lst.append(liebiao_biaoti_xpath_LineEdit)
        self.kongjian_dict['列表定位-dict'] = liebiao_biaoti_xpath_LineEdit

        liebiao_biaoti_xpath_LineEdit = QLineEdit()
        liebiao_biaoti_xpath_LineEdit.setText(self.kongjian_text_data['列表title-dict'])
        liebiao_biaoti_xpath_LineEdit.setGeometry(55, 20, 200, 20)
        shurukuang_lst.append(liebiao_biaoti_xpath_LineEdit)
        self.kongjian_dict['列表title-dict'] = liebiao_biaoti_xpath_LineEdit

        liebiao_url_xpath_LineEdit = QLineEdit()
        liebiao_url_xpath_LineEdit.setText(self.kongjian_text_data['列表url-dict'])
        liebiao_url_xpath_LineEdit.setGeometry(55, 20, 200, 20)
        shurukuang_lst.append(liebiao_url_xpath_LineEdit)
        self.kongjian_dict['列表url-dict'] = liebiao_url_xpath_LineEdit

        liebiao_url_xpath_LineEdit = QLineEdit()
        liebiao_url_xpath_LineEdit.setText(self.kongjian_text_data['列表url-拼接前段'])
        liebiao_url_xpath_LineEdit.setGeometry(55, 20, 200, 20)
        shurukuang_lst.append(liebiao_url_xpath_LineEdit)
        self.kongjian_dict['列表url-拼接前段'] = liebiao_url_xpath_LineEdit

        liebiao_publishtime_xpath_LineEdit = QLineEdit()
        liebiao_publishtime_xpath_LineEdit.setText(self.kongjian_text_data['列表时间-dict'])
        liebiao_publishtime_xpath_LineEdit.setGeometry(55, 20, 200, 20)
        shurukuang_lst.append(liebiao_publishtime_xpath_LineEdit)
        self.kongjian_dict['列表时间-dict'] = liebiao_publishtime_xpath_LineEdit
    ...
    def get_Data(self):
        self.baocun_Data()
        chuangjian_post_gui.chuangjian_post(self.kongjian_dict)
    ...
    def baocun_Data(self):
        with open("post_data.json", "r", encoding="utf-8") as f:
            ...
            old_data["起始url"] = self.kongjian_dict['起始url'].text()
            old_data["起始data_lst"] = self.kongjian_dict['起始data_lst'].toPlainText()
            old_data["列表定位-dict"] = self.kongjian_dict['列表定位-dict'].text()
            old_data["列表title-dict"] = self.kongjian_dict['列表title-dict'].text()
            old_data["列表url-dict"] = self.kongjian_dict['列表url-dict'].text()
            old_data["列表url-拼接前段"] = self.kongjian_dict['列表url-拼接前段'].text()
            old_data["列表时间-dict"] = self.kongjian_dict['列表时间-dict'].text()
