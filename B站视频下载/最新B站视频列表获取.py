...
def analysis_parms(info_json):
    ...
    now_count = int(info_json.get("data", {}).get("page", {}).get("pn")) * int(
        info_json.get("data", {}).get("page", {}).get("ps"))
    ...
    for each in lis:
        ...
        if dic_info["pubtime"]:
            dic_info["pubtime"] = datetime.datetime.fromtimestamp(int(str(dic_info["pubtime"])[:10])).strftime(
                "%Y-%m-%d %H:%M:%S")
...
def jian_time(created):
    time_end = int(time.time())
    interval = time_end - created
    return interval

def xieru_shuju(cursor, conn, aid, author, bvid, length, mid, pic, title, play, interval):
    cursor.execute(f"INSERT INTO blbl_video_data (VIDEO_AID, VIDEO_AUTHOR, VIDEO_BVID, VIDEO_LENGTH, VIDEO_MID, VIDEO_PIC, VIDEO_TITLE, video_play, video_interval) VALUES ('{aid}','{author}','{bvid}','{length}','{mid}','{pic}','{title}','{play}','{interval}');")
    conn.commit()  # 刷新数据，用来写入的
...
if __name__ == '__main__':
    ...
    for x in info:
        ...
        play = x['play_num']
        created = 0
        # interval = jian_time(created)
        interval = 0
        xieru_shuju(cursor, conn, aid, author, bvid, length, mid, pic, title, play, interval)
    print(len(info))
