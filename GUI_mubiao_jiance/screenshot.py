...
class Screenshot:
    def __init__(self, top, scale):
        ...
        time.sleep(0.5)
        # 全屏截图;
        self.img_screenshot = ImageGrab.grab((0, 0, self.top_screenshot.winfo_screenwidth(), self.top_screenshot.winfo_screenheight()))
        # # 保存截图;
        # self.img_screenshot.save('%s_%s@%s_%s.png' % (0, 0, self.top_screenshot.winfo_screenwidth(), self.top_screenshot.winfo_screenheight()))
        # # 打开截图;
        # self.img_screenshot = Image.open('%s_%s@%s_%s.png' % (0, 0, self.top_screenshot.winfo_screenwidth(), self.top_screenshot.winfo_screenheight()))
        "---------------------------------------------------窗口边框"
        # 初始化边框画布边框宽度;
        ...
        self.first_y = 0
        # 创建矩形,充当截图区域;
        self.canvas_screenshot.create_rectangle(0, 0, 0, 0, fill='red', outline='yellow', width=round(self.canvas_frame_frame_width), tags='L')
        "---------------------------------------------------放大窗口"
    ...
    def mouse_lift_event(self, event):
        ...
        if self.first_x - mouse_x > 0 and self.first_y - mouse_y > 0:
            ...
            self.screenshot_img = self.img_screenshot.crop((down_right_x, down_right_y, screenshot_top_left_x, screenshot_top_left_y))
        # 鼠标起始向左下截图;
        if self.first_x - mouse_x > 0 > self.first_y - mouse_y:
            down_right_x = screenshot_top_left_x - abs(self.first_x - mouse_x)
            down_right_y = screenshot_top_left_y + abs(self.first_y - mouse_y)
            self.screenshot_img = self.img_screenshot.crop((down_right_x, screenshot_top_left_y, screenshot_top_left_x, down_right_y))
        # 鼠标起始向右上截图;
        if self.first_x - mouse_x < 0 < self.first_y - mouse_y:
            down_right_x = screenshot_top_left_x + abs(self.first_x - mouse_x)
            down_right_y = screenshot_top_left_y - abs(self.first_y - mouse_y)
            self.screenshot_img = self.img_screenshot.crop((screenshot_top_left_x, down_right_y, down_right_x, screenshot_top_left_y))
        # 鼠标起始向右下截图;
        if self.first_x - mouse_x < 0 and self.first_y - mouse_y < 0:
            down_right_x = screenshot_top_left_x + abs(self.first_x - mouse_x)
            down_right_y = screenshot_top_left_y + abs(self.first_y - mouse_y)
            self.screenshot_img = self.img_screenshot.crop((screenshot_top_left_x, screenshot_top_left_y, down_right_x, down_right_y))
        # 保存截图;
        if down_right_x > 0 and down_right_y > 0:
            self.screenshot_img.save('jietu/%s_%s@%s_%s.png' % (screenshot_top_left_x, screenshot_top_left_y, down_right_x, down_right_y))
    ...
    def img_enlarge(self):
        if self.win_close == 0:
            ...
            # 抓取鼠标附近屏幕快照;
            img_cropping = self.img_screenshot.crop((mouse_x - mouse_range, mouse_y - mouse_range,  mouse_x + mouse_range, mouse_y + mouse_range))
            # 放大鼠标附近屏幕快照;
